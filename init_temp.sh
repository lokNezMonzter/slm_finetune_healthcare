...
# Run the container with persistence, heavy shared memory, and storage isolation
sudo docker run -d \
  --name $CONTAINER_NAME \
  --gpus all \
  --restart=always \
  ...
  -v /mnt/models/hf_cache:/huggingface_cache \
  -v /mnt/models/outputs:/workspace/outputs \
  -v /mnt/models/datasets:/data:ro \
  $IMAGE_NAME \
  sleep infinity
