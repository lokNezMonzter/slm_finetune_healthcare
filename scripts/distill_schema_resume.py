...
async def main():
    ...
    target_dataset = remaining_pool[:6000]

    # Build a set of already processed IDs
    processed_ids = set()
    try:
        with open("distillation_results.jsonl", "r") as f:
            for line in f:
        ...
        print(f"⚠️ [ERR] FILE NOT FOUND!!", flush=True)

    # Filter the queue
    pending_records = [rec for rec in target_dataset if str(rec["patient_uid"]) not in processed_ids]

    print(f"#️⃣ Total Target: 5555\n#️⃣ Already Processed: {len(processed_ids)}\n#️⃣ Resuming with: {len(pending_records)}\n", flush=True)
    ...
    print(f"▶️ Resuming batch extraction of {len(dataset)} records...", flush=True)
    ...
    with open("distillation_results_v2.jsonl", "a") as f:
